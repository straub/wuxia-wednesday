<template>
  <OModal
    ...
    :active="isActive"
    @update:active="(newValue: boolean) => emit('update:isActive', newValue)"
  >
    <OField label="Find path to movie" label-size="large">
      <OAutocomplete
        ref="input"
        :data="data"
...
const emit = defineEmits(['update:isActive', 'select']);

const input = ref<HTMLInputElement>();

watch(() => props.isActive, () => nextTick(() => input.value?.focus()));
